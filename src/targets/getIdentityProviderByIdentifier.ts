...
export const GetIdentityProviderByIdentifier =
  ({ cognito }: Pick<Services, 'cognito'>): GetIdentityProviderByIdentifierTarget =>
  async (ctx, req) => {
    if (!req.UserPoolId) throw new MissingParameterError('UserPoolId');
    if (!req.IdpIdentifier) throw new MissingParameterError('IdpIdentifier');

    const userPool = await cognito.getUserPool(ctx, req.UserPoolId);
    const identityProvider = await userPool.getIdentityProviderByIdentifier(ctx, req.IdpIdentifier);
    if (!identityProvider) {
