...
describe("PostConfirmation trigger", () => {
  ...
  describe.each(["PostConfirmation_ConfirmSignUp", "PostConfirmation_ConfirmForgotPassword"])(
    "%s",
    (source) => {
      ...
      describe("when lambda invoke succeeds", () => {
        it("quietly completes", async () => {
          ...
          expect(mockLambda.invoke).toHaveBeenCalledWith(TestContext, "PostConfirmation", {
            clientId: "clientId",
