...
describe('CognitoServiceFactory', () => {
  it('creates a database for clients', async () => {
    ...
    expect(mockDataStoreFactory.create).toHaveBeenCalledWith(TestContext, 'clients', {
      Clients: {},
    });
  });
});
...
describe('Cognito Service', () => {
  ...
  describe('getUserPoolForClientId', () => {
    ...
    it('creates a user pool by the id in the client config', async () => {
      ...
      const userPool = await cognitoClient.getUserPoolForClientId(TestContext, 'testing');

      expect(clientsDataStore.get).toHaveBeenCalledWith(TestContext, ['Clients', 'testing']);
      expect(mockUserPoolServiceFactory.create).toHaveBeenCalledWith(
        TestContext,
        clientsDataStore,
